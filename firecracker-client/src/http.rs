...
async fn send_request(
    ...
    let (mut sender, conn) = http1::handshake(TokioIo::new(stream)).await.context("HTTP handshake failed")?;
    ...
    let request = Request::builder()
        ...
        .body(Full::new(Bytes::from(bytes))).context("failed to build request")?;

    let response = sender.send_request(request).await.context("failed to send request to firecracker")?;

    if !response.status().is_success() {
        let status = response.status();
        let bytes = response.into_body().collect().await.context("failed to read response body")?.to_bytes();
