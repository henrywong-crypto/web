...
function validatePayload(eventName: string, payload: unknown): SseEvent | null {
  switch (eventName) {
    ...
    case "ask_user_question":
      if (!hasFields(payload, ["request_id", "task_id", "questions"])) return null;
      return { type: "ask_user_question", payload: payload as SseAskUserQuestion };
    case "tool_result":
...
export function attachEventSourceListeners(
  ...
  add("ask_user_question", (e) => {
    const p = safeParse(e.data);
    const event = p !== undefined ? validatePayload("ask_user_question", p) : null;
    if (event) pushEvent(event);
  });
  add("tool_result", (e) => {
    const p = safeParse(e.data);
