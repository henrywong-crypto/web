...
interface QuickSettingsPanelProps {
  ...
  onToggle: <K extends keyof UiPreferences>(key: K, value: UiPreferences[K]) => void;
}

const TOGGLES: { key: keyof UiPreferences; label: string; description: string }[] = [
    ...
    <>
      ...
      <div className="fixed inset-y-0 right-0 z-50 w-72 border-l border-border bg-card shadow-xl">
        <div className="flex h-11 items-center justify-between border-b border-border px-4">
          <span className="text-sm font-semibold text-foreground">Quick Settings</span>
          <button
            onClick={onClose}
        ...
        <div className="space-y-1 p-4">
          {TOGGLES.map((t) => (
            <label key={t.key} className="flex items-center justify-between rounded-lg px-2 py-3">
              <div>
