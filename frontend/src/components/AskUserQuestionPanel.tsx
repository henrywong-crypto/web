...
> = {
  low: {
    ...
    badge: "bg-emerald-500/10 text-emerald-500 border-emerald-200 dark:border-emerald-800/50",
    badgeText: "Safe",
    label: "text-emerald-600 dark:text-emerald-400",
    selected: "border-emerald-300 bg-emerald-50/80 ring-1 ring-emerald-200/50 dark:border-emerald-600 dark:bg-emerald-900/25",
    selectedDesc: "text-emerald-600/70 dark:text-emerald-300/70",
    check: "text-emerald-500",
    accent: "from-emerald-500/10 to-teal-500/10",
    kbd: "bg-emerald-500",
    otherBorder: "border-emerald-300 bg-emerald-50/80 ring-1 ring-emerald-200/50 dark:border-emerald-600 dark:bg-emerald-900/25",
    stepDot: "bg-emerald-500",
    headerBadge: "border-emerald-100 bg-emerald-50 text-emerald-600 dark:border-emerald-800/50 dark:bg-emerald-900/30 dark:text-emerald-400",
    submitBtn: "from-emerald-600 to-emerald-500",
  },
  medium: {
    gradient: "from-amber-500 via-yellow-400 to-orange-400",
    badge: "bg-amber-500/10 text-amber-500 border-amber-200 dark:border-amber-800/50",
    badgeText: "Review",
    label: "text-amber-600 dark:text-amber-400",
    selected: "border-amber-300 bg-amber-50/80 ring-1 ring-amber-200/50 dark:border-amber-600 dark:bg-amber-900/25",
    selectedDesc: "text-amber-600/70 dark:text-amber-300/70",
    check: "text-amber-500",
    accent: "from-amber-500/10 to-yellow-500/10",
    kbd: "bg-amber-500",
    otherBorder: "border-amber-300 bg-amber-50/80 ring-1 ring-amber-200/50 dark:border-amber-600 dark:bg-amber-900/25",
    stepDot: "bg-amber-500",
    headerBadge: "border-amber-100 bg-amber-50 text-amber-600 dark:border-amber-800/50 dark:bg-amber-900/30 dark:text-amber-400",
  ...
  high: {
    ...
    selected: "border-red-300 bg-red-50/80 ring-1 ring-red-200/50 dark:border-red-600 dark:bg-red-900/25",
    selectedDesc: "text-red-600/70 dark:text-red-300/70",
    check: "text-red-500",
    accent: "from-red-500/10 to-rose-500/10",
    kbd: "bg-red-500",
    otherBorder: "border-red-300 bg-red-50/80 ring-1 ring-red-200/50 dark:border-red-600 dark:bg-red-900/25",
    stepDot: "bg-red-500",
    headerBadge: "border-red-100 bg-red-50 text-red-600 dark:border-red-800/50 dark:bg-red-900/30 dark:text-red-400",
  ...
  return (
    ...
    >
      <div className="relative overflow-hidden rounded-2xl border border-gray-200/80 bg-white shadow-lg dark:border-gray-700/50 dark:bg-gray-800/90">
        <div className={`absolute left-0 right-0 top-0 h-[2px] bg-gradient-to-r ${rs.gradient}`} />

        <div className="px-4 pb-2 pt-3.5">
          <div className="mb-1.5 flex items-center gap-2.5">
            <div className="relative flex-shrink-0">
              <div className={`flex h-6 w-6 items-center justify-center rounded-lg bg-gradient-to-br ${rs.accent}`}>
                <svg
              ...
              <div className={`absolute -right-0.5 -top-0.5 h-2 w-2 animate-pulse rounded-full ${risk === "high" ? "bg-red-400" : risk === "medium" ? "bg-amber-400" : "bg-cyan-400"}`} />
            </div>
            <div className="flex min-w-0 flex-1 items-center gap-2">
              <span className="text-xs font-medium uppercase tracking-wide text-gray-400 dark:text-gray-500">
                Claude needs your input
              </span>
              <span className={`inline-flex items-center rounded border px-1.5 py-px text-xs font-semibold uppercase tracking-wider ${rs.badge}`}>
                {rs.badgeText}
              </span>
              {q.header && (
                <span className={`inline-flex items-center rounded border px-1.5 py-px text-xs font-semibold uppercase tracking-wider ${rs.headerBadge}`}>
                  {q.header}
                </span>
              )}
            </div>
