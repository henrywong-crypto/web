...
test.describe("Empty state and onboarding", () => {
  test("shows blank state message when no conversations exist", async ({ page }) => {
  ...
  test("composer is focused on load", async ({ page }) => {
    const ctrl = await setupApp(page);
    await expect(page.locator('textarea[placeholder="Message Claude…"]')).toBeFocused();
  });
});
...
test.describe("Tool rendering edge cases", () => {
  test("shows tool card for unknown tool name", async ({ page }) => {
    ...
    ctrl.sendSseEvents(
      sse.withTool("t1", "custom_unknown_tool", { arg: "val" }, "tool output", "done", "sess-1"),
    );
...
test.describe("Session resumption", () => {
  test("resumes conversation with existing session_id", async ({ page }) => {
    const conv = makeConversation({ conversationId: "conv-resume", sessionId: "sess-existing", projectDir: "/home/ubuntu" });
    const ctrl = await setupApp(page, {
      conversations: [conv],
...
test.describe("CSRF token rotation", () => {
  ...
  test("uses rotated CSRF token from response header in subsequent request", async ({ page }) => {
    const ctrl = await setupApp(page);
