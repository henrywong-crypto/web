...
pub(crate) async fn delete_handler(
    ...
    if real_path == PathBuf::from(&state.config.upload_dir) {
    ...
    Box::pin(async move {
        ...
        for entry in &entries {
            ...
            if entry.metadata().file_type().is_dir() {
                ...
                sftp.remove_file(entry_str).await.context("failed to remove file")?;
            }
        }
        sftp.remove_dir(path_str).await.context("failed to remove directory")?;
        Ok(())
    })
}
