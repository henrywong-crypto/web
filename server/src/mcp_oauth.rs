...
pub(crate) async fn register_handler(
    ...
    let reg_resp: RegisterResponse = resp.json().context("failed to parse registration response")?;
...
pub(crate) async fn start_handler(
    ...
    if !is_safe_url(&body.authorization_endpoint) {
        return Ok(
            (StatusCode::BAD_REQUEST, "Invalid authorization endpoint").into_response(),
        );
    }
...
pub(crate) async fn callback_handler(
    ...
    let tokens: TokenResponse =
        token_resp.json().context("failed to parse token response")?;

    info!("mcp oauth token exchange successful for server: {}", oauth_session.server_name);
    ...
    info!("mcp oauth: successfully wrote server '{}' config to VM", oauth_session.server_name);
...
mod tests {
    ...
    async fn discover_no_oauth() {
        ...
        let responses: Vec<MockHttpResponse> = (0..5)
            .map(|_| mock_http_response(404, ""))
            .collect();
    ...
    async fn start_rejects_unsafe_token_endpoint() {
        let session = tower_sessions::Session::new(None, std::sync::Arc::new(tower_sessions::MemoryStore::default()), None);
        ...
        let resp = start_handler(test_user_vm(), session, body)
            .await
            .unwrap();
        assert_eq!(resp.status(), StatusCode::BAD_REQUEST);
    }
