...
pub(crate) async fn handle_chat_upload(
    ...
    let remote_path = stream_chat_attachment(&mut multipart, &sftp, &state.config.upload_dir).await?;
...
async fn stream_chat_attachment(multipart: &mut Multipart, sftp: &SftpSession, upload_dir: &Path) -> Result<PathBuf> {
